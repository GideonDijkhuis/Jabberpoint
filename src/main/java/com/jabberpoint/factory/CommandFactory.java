 ...
 * - Single Responsibility Principle: Only responsible for creating command objects
 * - Open/Closed Principle: New command types can be added without modifying existing code
 * - Dependency Inversion Principle: Depends on abstractions (Command, Receiver interfaces)
 * - Interface Segregation Principle: Creates specific commands that implement focused interfaces
 * - Liskov Substitution Principle: All commands can be used interchangeably through Command interface
 *
 * Factory for creating various commands used in the application.
