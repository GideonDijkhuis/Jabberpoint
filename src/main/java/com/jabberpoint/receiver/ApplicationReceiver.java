 ...
 *
 * SOLID Principles: - Single Responsibility Principle: Handles only application-level commands (exit, about). -
 * Open/Closed Principle: New application commands can be added without modifying existing code. - Liskov Substitution
 * Principle: Properly implements the Receiver interface. - Interface Segregation Principle: Implements the Receiver
 * interface with methods relevant to application commands. - Dependency Inversion Principle: Depends on abstractions
 * (Receiver interface) rather than concrete implementations.
 */
